...
function ReportExpand(props) {
  ...
  return (
    <article className="border-2 border-brand-turquoise-50 rounded-md my-3 sm:max-w-2xl sm:mx-auto">
      <button
        onClick={handeChange}
        className="rounded-md flex flex-row w-full gap-2 bg-brand-turquoise-00 p-2"
      >
      ...
      <div className={expanded ? styles.open : styles.closed}>
        ...
        <p>{props.help}</p>
        <Link href={`/wiki/${props.id}`}>Read more here</Link>
      </div>
    </article>
  );
}
